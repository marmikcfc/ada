...
export const ThreadedChatWindow: React.FC<ThreadedChatWindowProps> = ({
  ...
  const threadListStyles: CSSProperties = {
    ...
    borderRight: threadListPosition === 'left' ? `1px solid ${cssVars['--geui-color-border']}` : 'none',
    borderLeft: threadListPosition === 'right' ? `1px solid ${cssVars['--geui-color-border']}` : 'none',
  ...
  return (
    <div className={`geui-threaded-chat-window ${className}`} style={containerStyles}>
      {/* Thread List */}
