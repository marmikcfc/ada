...
const BubbleWidget: React.FC<BubbleWidgetProps> = ({
  ...
  const micControlStyles: CSSProperties = {
    ...
    opacity: isHovered ? 1 : 0,
    pointerEvents: isHovered ? 'auto' : 'none',
    backgroundColor: isMicActive ? cssVars['--geui-color-primary'] : cssVars['--geui-color-surface'],
    color: isMicActive ? '#ffffff' : cssVars['--geui-color-text'],
    marginBottom: '-25px',
  };
  ...
  const hoverStyles = `
    ...
    .mic-active:hover {
      background-color: ${cssVars['--geui-color-primary']} !important;
      opacity: 0.9;
    }
  ...
  return (
    <>
      ...
      <div 
        className={`geui-bubble-widget ${className}`}
