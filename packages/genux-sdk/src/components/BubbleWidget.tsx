...
const BubbleWidget: React.FC<BubbleWidgetProps> = ({
  ...
  const micControlStyles: CSSProperties = {
    ...
    opacity: isHovered ? 1 : 0,
    pointerEvents: isHovered ? 'auto' : 'none',
    backgroundColor: isMicActive ? cssVars['--genux-color-primary'] : cssVars['--genux-color-surface'],
    color: isMicActive ? '#ffffff' : cssVars['--genux-color-text'],
    marginBottom: '-25px',
  };
  ...
  return (
    <>
      ...
      <div 
        className={`genux-bubble-widget ${className}`}
