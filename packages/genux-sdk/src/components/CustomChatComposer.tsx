...
const CustomChatComposer: React.FC<ExtendedChatComposerProps> = ({
  ...
  const composerContainerStyles: CSSProperties = {
    borderTop: `1px solid ${cssVars['--genux-color-border']}`,
  ...
  const innerContainerStyles: CSSProperties = {
    ...
    border: `1px solid ${cssVars['--genux-color-border']}`,
    overflow: 'hidden',
  };
  ...
  const textareaStyles: CSSProperties = {
    flex: 1,
    border: 'none',
    padding: `${cssVars['--genux-spacing-md']} ${cssVars['--genux-spacing-lg']} ${cssVars['--genux-spacing-md']} ${cssVars['--genux-spacing-md']}`,
    resize: 'none',
    outline: 'none',
  ...
  const actionsContainerStyles: CSSProperties = {
    display: 'flex',
    alignItems: 'center',
    marginLeft: cssVars['--genux-spacing-sm'],
...
const VoiceButtonComponent = componentOverrides?.VoiceButton || DefaultVoiceButton;
  ...
  return (
    <div className={`genux-chat-composer ${className}`} style={composerContainerStyles}>
      <div className="genux-composer-container" style={innerContainerStyles}>
        <textarea
          ref={textareaRef}
          className="genux-composer-input"
        ...
        />
        <div className="genux-composer-actions" style={actionsContainerStyles}>
          {/* Send Button */}
          <button
            className={`genux-send-button ${(disabled || !message.trim() || isVoiceActive || isLoading) ? 'genux-disabled' : ''}`}
            onClick={handleSend}
          ...
          >
            {isLoading ? (
              ...
              <svg 
                ...
                viewBox="0 0 24 24" 
                fill="none" 
                stroke={disabled || !message.trim() || isVoiceActive ? '#adb5bd' : cssVars['--genux-color-primary']}
                strokeWidth="2.5" 
                strokeLinecap="round" 
                strokeLinejoin="round"
                style={{ 
