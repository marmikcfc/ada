...
export const ThreadList: React.FC<ThreadListProps> = ({
  ...
  const threadItemStyles = (isActive: boolean): CSSProperties => ({
    ...
    cursor: 'pointer',
    backgroundColor: isActive ? cssVars['--genux-color-primary'] : 'transparent',
    color: isActive ? '#ffffff' : cssVars['--genux-color-text'],
  ...
  return (
    <div className={`genux-thread-list ${className}`} style={containerStyles}>
      {/* New Thread Button */}
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <span style={{ fontSize: cssVars['--genux-font-size-sm'], fontWeight: '500' }}>
          {isCreatingThread ? 'Creating...' : 'New conversation'}
        </span>
      </div>

      {/* Thread List */}
      <div style={{ flex: 1, overflowY: 'auto', padding: cssVars['--genux-spacing-sm'] }}>
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: cssVars['--genux-spacing-lg'] }}>
            Loading threads...
          </div>
        ) : threads.length === 0 ? (
          <div style={{ 
      ...
      <style>{`
        .genux-thread-list .thread-item:hover .thread-actions {
          opacity: 1 !important;
        }
      `}</style>
    </div>
